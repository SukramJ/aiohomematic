...
@pytest.mark.asyncio
async def test_session_recorder(session_player_ccu) -> None:
    """Test the session recorder."""
    assert session_player_ccu
    lm_methods = session_player_ccu.get_latest_response_by_method(rpc_type=RPCType.XML_RPC, method="system.listMethods")
    assert lm_methods
    assert len(lm_methods) == 1

    assert session_player_ccu
    pd_methods = session_player_ccu.get_latest_response_by_method(
        rpc_type=RPCType.XML_RPC, method="getParamsetDescription"
    )
    assert pd_methods
    assert len(pd_methods) == 3561

    list_methods = session_player_ccu.get_latest_response_by_params(
