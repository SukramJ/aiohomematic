...
class TestServiceStatsRegistry:
    """Tests for the service stats global registry."""
    ...
    def test_clear_all_service_stats(self) -> None:
        ...
        record_service_call(
            central_name="central_a",
            method_name="method1",
            duration_ms=50.0,
            had_error=False,
        )
        record_service_call(
            central_name="central_b",
            method_name="method2",
            duration_ms=100.0,
            had_error=False,
        )
        ...
        stats_a = get_service_stats(central_name="central_a")
        stats_b = get_service_stats(central_name="central_b")
    ...
    def test_clear_service_stats_specific_central(self) -> None:
        ...
        # Clear only central A
        clear_service_stats(central_name="central_a")

        # Check that central A is cleared but central B remains
        stats_a = get_service_stats(central_name="central_a")
        stats_b = get_service_stats(central_name="central_b")

        assert len(stats_a) == 0
        assert "method2" in stats_b
    ...
    def test_multi_central_isolation(self) -> None:
        ...
        assert "method1" in stats_a
        assert "method2" not in stats_a
        assert "method2" in stats_b
        assert "method1" not in stats_b
    ...
    def test_record_and_get_service_stats(self) -> None:
        ...
        # Get stats
        stats = get_service_stats(central_name="test_central")

        assert "turn_on" in stats
        assert "turn_off" in stats
        assert stats["turn_on"].call_count == 2
        assert stats["turn_on"].error_count == 0
        assert stats["turn_off"].call_count == 1
        assert stats["turn_off"].error_count == 1
