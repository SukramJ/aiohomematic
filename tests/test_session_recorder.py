...
@pytest.mark.asyncio
async def test_session_recorder(session_recorder_from_full_session_ccu) -> None:
    """Test the session recorder."""
    assert session_recorder_from_full_session_ccu
    lm_methods = session_recorder_from_full_session_ccu.get_latest_response_by_method(
    ...
    assert session_recorder_from_full_session_ccu
    pd_methods = session_recorder_from_full_session_ccu.get_latest_response_by_method(
        rpc_type=RPCType.XML_RPC, method="getParamsetDescription"
    )
    assert pd_methods
    assert len(pd_methods) == 3561

    list_methods = session_recorder_from_full_session_ccu.get_latest_response_by_params(
