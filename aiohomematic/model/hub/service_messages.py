...
"""Module for hub service messages sensor."""
...
from aiohomematic.const import HUB_ADDRESS, DataPointCategory, HubValueType, ServiceMessageData
...
_SERVICE_MESSAGES: Final = "SERVICE_MESSAGES"
...
class HmServiceMessagesSensor(CallbackDataPoint, HubSensorDataPointProtocol, PayloadMixin):
    ...
    def __init__(
        ...
        *,
        ...
        unique_id: Final = generate_unique_id(
            ...
            address=HUB_ADDRESS,
            parameter=slugify(_SERVICE_MESSAGES_SENSOR_NAME),
        )
        self._name_data: Final = get_hub_data_point_name_data(
            channel=None,
            legacy_name=_SERVICE_MESSAGES_SENSOR_NAME,
    ...
    def additional_information(self) -> dict[str, Any]:
        ...
        if self._messages:
            ainfo[_SERVICE_MESSAGES] = [
                f"{m.device_name}: {m.display_name} ({m.msg_type_name})" if m.device_name else m.display_name
    ...
    def update_data(self, *, messages: tuple[ServiceMessageData, ...], write_at: datetime) -> None:
        """Update the data point with new service messages."""
        new_count = len(messages)
        if self._cached_message_count != new_count or self._messages != messages:
            self._cached_message_count = new_count
    ...
    def _get_path_data(self) -> PathData:
        ...
        return HubPathData(name=slugify(_SERVICE_MESSAGES_SENSOR_NAME))
