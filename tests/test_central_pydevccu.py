...
class TestCentralPyDevCCU:
    ...
    async def test_central_full(self, central_unit_pydevccu_full) -> None:  # noqa: C901
        ...
        assert usage_types[DataPointUsage.CDP_VISIBLE] == 152
        assert usage_types[DataPointUsage.DATA_POINT] == 4011
        assert usage_types[DataPointUsage.NO_CREATE] == 4397

        assert len(ce_channels) == 135
        assert len(data_point_types) == 6
        assert len(parameters) == 252
